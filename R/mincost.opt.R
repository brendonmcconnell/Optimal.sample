...
#' @title Cost minimizing sample designs for cluster Randomized Control Trials with only an endline measurement subject to a power constraint.
#' @description This function delivers the number of clusters and the number of units (individuals, firms, etc) within cluster that minimizes the total costs
#' subject to a power constraint in a cluster Randomized Control Trial (RCT). The function assumes that the outcome variable is continuous and has only been measured at endline.
...
#' @param beta Vector. Maximum level of power of implementing the RCT.
...
#'
#' @return Returns a matrix of size (13 x number of Scenarios). Each scenario is a combination of specify parameters, and fixed and variable costs per unit. For each scenario the matrix provides the following components: \describe{
#'
...
#'
#' \item{Cost}{This is the vector of the minimum cost of the RCT with the optimum number of clusters and units provided by this function.}
#'
#'}
#'
#'
...
mincost.opt <- function(delta,
  ...
  min.costs.nA <- mincost.nA (delta = delta,
                              sigma = sigma,
                              rho = rho,
                              alpha = alpha,
                              beta = beta,
  ...
  ### Return final matrix with all the parameters and the minimum costs or Export results to excel
  ...
  if (is.null(output)){
    ...
    for (i in 1:nobjects) {
      if (i == 1)
        write.xlsx2(as.data.frame(min.costs.nA), file = paste0(objects[[1]],'.xlsx'), col.names=TRUE, row.names=TRUE, append=FALSE)
    }
  }
